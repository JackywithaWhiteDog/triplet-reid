...
from nets.lunet import lunet_50, resnet_arg_scope
...
def endpoints(image, is_training):
    ...
    with slim.arg_scope(resnet_arg_scope(batch_norm_decay=0.9, weight_decay=0.0)):
        _, endpoints = lunet_50(image, num_classes=None, is_training=is_training, global_pool=True)

    endpoints['model_output'] = endpoints['global_pool'] = tf.reduce_mean(
        endpoints['lunet_50/block4'], [1, 2], name='pool5')

    return endpoints, 'lunet_50'
